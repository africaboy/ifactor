...
function mousey() {
	/*
	 * var myobj = document.getElementById("divFlyBar"); var bodyww =
	 * document.body.clientWidth; var objww = myobj.clientWidth; var y = (bodyww -
	 * objww) / 2; return y < 0 ? 50 : y;
	 */
	// return event.clientY;
	if (event) {
		return getT(event.srcElement) + event.srcElement.offsetHeight
	} else {
...
function mousex() {
	/*
	 * var myobj = document.getElementById("divFlyBar"); var bodyhh =
	 * document.body.clientHeight; var objhh = myobj.clientHeight; x = (bodyhh -
	 * objhh) / 2; return x < 0 ? 50 : x;
	 */
	// return event.clientX;
	if (event) {
		return getL(event.srcElement);
	} else {
		var myobj = document.getElementById("divFlyBar");
		var bodyhh = document.body.clientHeight;
		var objhh = myobj.clientHeight;
		var x = (bodyhh - objhh) / 2;
		return x < 0 ? 50 : x;
	}
}
...
function dialog4submit(u, rnt, title, width, height) {
	...
	var str = '<iframe name="ss" id="ss" frameborder="0" width="100%" height="'
			+ height + '" scrolling="auto" src="' + context
			+ '/system/blank.jsp"></iframe>';
...
function myload_flybar(x, y) {
	divFlyBar.style.pixelLeft = x;
	divFlyBar.style.pixelTop = y;
...
function initDialog(width, height) {
	...
	str += '<td style="background-color:#ffffff;" align=left><div id=flyTailerHolder name=flyTailerHolder></div><div id=flyTailerHolder1 name=flyTailerHolder1></div></td>';
	str += '</tr></table></div>';
