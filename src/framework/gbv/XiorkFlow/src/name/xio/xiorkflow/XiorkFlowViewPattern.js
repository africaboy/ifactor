function XiorkFlowViewPattern(ui) {
	...
	this.statusPanel.setText("流程绘图工具");
	...
	this.xiorkFlowWrapper = new XiorkFlowWrapper(this.xiorkFlowViewer, model,
			this.stateMonitor, this.statusPanel);
...
XiorkFlowViewPattern.prototype.update = function(observable, arg) {
	if (arg == ButtonGroup.PRESSED_CHANGED) {
		...
		if (observable == this.getToolBar().getViewerPatternButtonGroup()) {
			var pressedButtonModel = this.getToolBar()
					.getViewerPatternButtonGroup().getPressedButtonModel();
...
function XiorkFlowViewerToolBar() {
	...
	if (insid != "") {
		/*var label = new Label();
		label.setInnerText("流程类别:");
		label.setFontWeight("bold");
		this.add(label);
		
		var label = new Label();
		label.setInnerText(flowname);
		this.add(label);
		*/
		
		var label = new Label();
		label.setInnerText("运行实例:");
		label.setFontWeight("bold");
		this.add(label);
		
		var label1 = new Label();
		var labelTxt = "《" +insname+ "》";
		label1.setInnerText(labelTxt);
		this.add(label1);
		
		var label = new Label();
		label.setInnerText("创建人/部门:");
		label.setFontWeight("bold");
		this.add(label);
		
		var label1 = new Label();
		var labelTxt = creator + "/" + unit;
		label1.setInnerText(labelTxt);
		this.add(label1);
		
		
		var label = new Label();
		label.setInnerText("创建时间:");
		label.setFontWeight("bold");
		this.add(label);
		
		var label1 = new Label();
		var labelTxt = createtime;
		label1.setInnerText(labelTxt);
		this.add(label1);
	}else{
		var label = new Label();
		label.setInnerText("流转过程图");
		label.setFontWeight("bold");
		this.add(label);
	}
