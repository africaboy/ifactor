 ...
 * (C) 2003-2009 Frederico Caldeira Knabben
 *  == BEGIN LICENSE ==
 * 
...
/**
 * Replaces a tag with another one, keeping its contents: for example TD --> TH,
 * and TH --> TD. input: the original node, and the new tag name
...
function autoSetObject(oActiveEl, colName) {
	/* 设置默认字段名称 */
	if (GetAttribute(oActiveEl, 'colname') == "") {
		GetE('colname').value = colName;
	} else {
		GetE('colname').value = GetAttribute(oActiveEl, 'colname');
	}

	/* 设置默认表单项名称 */
	GetE('txtName').value = GetAttribute(oActiveEl, 'collabel');
	
	/* 设置字段类型 */
	if (GetAttribute(oActiveEl, 'coltype') != "") {
		GetE('coltype').value = GetAttribute(oActiveEl, 'coltype');
	}

	/* 设置默认字段大小 */
	if (GetAttribute(oActiveEl, 'colsize') == "") {
		GetE('colsize').value = "20";
	} else {
		GetE('colsize').value = GetAttribute(oActiveEl, 'colsize');
	}
}