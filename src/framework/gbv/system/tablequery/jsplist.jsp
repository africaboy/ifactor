...
<link rel="stylesheet" type="text/css" href="<%=context%>/system/tablequery/css/querylist4jsp.css" />
<script src="<%=context %>/system/tablequery/js/querylist4jsp.js"></script>
...
<body onload="javascript:initTQJspList('<%=queryKey %>', function(){addMeMix('<%=queryKey %>');},'<%=orderby %>', '<%=orderbyKey %>', '<%=pageSize %>');">
<%if(!query.iname().equals("")){ %><div style="margin-top:15px;"><h1>&nbsp;&nbsp;<%=query.iname() %></h1></div><%} %>
...
<table class="tab4list" id="<%=queryKey %>" align="center" width="98%" style="margin:auto;">
     <tr>  
     <td class="td1" align="center"><%if(query.ishowchk()){ %><input title="全部选择/取消" type="checkbox" name="chkall" value="" onclick="javascript:checkAllItems(this);"><%} %>&nbsp;</td>
     <%
