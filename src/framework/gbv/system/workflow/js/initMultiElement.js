...
function addMultiButton(tab) {
	if (findMultiTableMAddtype(tab) == 1 || findMultiTableMDeltype(tab) == 1) {
		var label = "";
		var table = document.getElementById(tab);
		if (findMultiTableMAddtype(tab) == 1) {
			label += "<a href=\"javascript:addMultiItem('" + table.id
					+ "');\">添加</a>&nbsp;";
		}

		if (findMultiTableMDeltype(tab) == 1) {
		...
		for (var i = 0; i < chkrows.length; i++) {
			...
			if (chk == "yes") {
				if (document.getElementById(ky + "_" + c)
					...
					r = deleteItem(t,
							document.getElementById(ky + "_" + c).value) == 0;
					if (!r) {
						alert("删除数据项失败");
					}
				} else {
...
function deleteItem(t, id) {
	var url = context + "/system/flow.do?method=deleteitem&tableName=" + t
			+ "&id=" + id;
