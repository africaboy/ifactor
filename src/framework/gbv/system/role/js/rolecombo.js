...
/**
 * 获得角色列表combo(返回值为角色key)
 * 
 * @return {}
 */
function getRoleSelectComboK(combolabel, returnIdField, returnNameField,
		returnTypeField, defaultValue, allowBlank) {
	var comboxid = returnIdField + '_';
	...
	var RoleListGrid = new Ext.grid.GridPanel({
				...
				columns : [sm, {
							...
							'role-language', 'list-name'),
							width : 150,
							sortable : true,
							dataIndex : 'name'
						}, {
							header : grooveTranslator.getLangLabel(
							'role-language', 'list-desc'),
							width : 150,
							sortable : true,
							dataIndex : 'memo'
						}],
 ...
 * @return {}
 */
function getRoleSingleSelectComboK(combolabel, returnIdField, returnNameField,
		returnTypeField, defaultValue, allowBlank) {
	var comboxid = returnIdField + '_';
