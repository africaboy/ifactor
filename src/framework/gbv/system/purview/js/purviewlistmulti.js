...
var purviewList4multiArea;
var purviewList4multiStore;
var purviewList4multiGrid;
 ...
 *            areaid
 */
function purviewList4multi(oid, oname, otype, otname, areaid, pt) {
	if (purviewList4multiGrid) {
	...
	purviewList4multiArea = Ext.get(areaid);

	purviewList4multiStore = new Ext.data.JsonStore({
				idProperty : 'id',
				remoteSort : false,
	...
	purviewList4multiGrid = new Ext.grid.EditorGridPanel({
				...
				border : false,
				height : purviewList4multiArea.getHeight(),
				autoWidth : true,
				viewConfig : {
					forceFit : true
				},
	...
	purviewList4multiGrid.render(areaid);
}
	...
	if (ids != '') {
		...
		for (var i = 0; i < idstr.length; i++) {
			if (!hasSelectedMultiPurview(idstr[i], typestr[i])) {
				var Plant = purviewList4multiGrid.getStore().recordType;
				...
				purviewList4multiGrid.getStore().add(p);
			}
		}
...
function removeMultiPurviewObjs(btn) {
	if (btn == 'yes') {
		var index = purviewList4multiGrid.getSelectionModel().getSelectedCell();

		var record = purviewList4multiGrid.getStore().getAt(index[0]);
		...
		if (ids != '') {
			...
			Ext.Ajax.request({
						...
						success : function(response) {
							Ext.MessageBox.hide();
							purviewList4multiGrid.getStore().remove(record);
...
function hasSelectedMultiPurview(id, type) {
	...
	var total = purviewList4multiStore.getCount();// 数据行数
	for (var j = 0; j < total; j++) {
		var record = purviewList4multiStore.getAt(j);
