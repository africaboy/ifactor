 ...
 *            allowBlank
 */
function getOQueryWindow(key, func, setFunc, returnIdField, pointId, pointName,
		allowBlank) {
	var querykey = key;
	...
	var url = context + '/system/objectarg.do?method=modelcmp&querykey='
			+ querykey + '&comptype=OQWindowList&' + param;
	...
	if (json.success) {
		...
		grid.on('click', function(e) {
					 ...
					 * 
					 * if (record) { if (Ext.getCmp(returnIdField)) {
					 * Ext.getCmp(returnIdField).setValue(record .get(pointId)); }
					 * 
 ...
 * @return {}
 */
function renderOQWindowListHeader(value, cellmeta, record, rowIndex,
		columnIndex, store) {
	return value;
}