...
<script src="<%=context%>/system/purview/js/purviewlistmulti.js"></script>
<script src="<%=context%>/system/wordbook/js/wblist.js"></script>
<%}else{ %>		
<script src="<%=context%>/system/purview/js/purviewlist.js"></script>
...
<script>
   Ext.onReady(function(){
     <%if(mode.equals("multi")){ %>
     purviewList4multi('<%=oid%>', '<%=oname%>', 'directory', 'APP_INFO_DIRECTORY', 'tree-div','<%=type%>');
     <%}else{%>
     purviewlist('<%=oid%>', '<%=oname%>', 'directory', 'APP_INFO_DIRECTORY', 'tree-div', '<%=type%>');
     <%}%>
});
</script>
<body>
<div id="tree-div" style="border:1px #ccc solid;width:700px;height:400px"></div>
</body>
</html>
