...
<%
	...
	/*待授权对象名称*/
	String oname = StringTool.checkString(cbh.getResource()
			.get("oname"));
...
<link rel="StyleSheet"
	...
	type="text/css" />
<link rel="StyleSheet"
	href="<%=context%>/system/group/css/groupmanage.css" type="text/css" />
<%
if (mode.equals("multi")) {
%>
<script src="<%=context%>/system/purview/js/purviewlistmulti4tab.js"></script>
<script src="<%=context%>/system/wordbook/js/wblist.js"></script>
<%
} else {
%>
<script src="<%=context%>/system/purview/js/purviewlist4tab.js"></script>
...
<script>
   Ext.onReady(function(){
     <%if(mode.equals("multi")){ %>
     var item = purviewList4multi('<%=oid%>', '<%=oname%>', 'directory', 'APP_INFO_DIRECTORY', '<%=type%>', '权限设置Tab');
     <%}else{%>
     var item = purviewlist('<%=oid%>', '<%=oname%>', 'directory', 'APP_INFO_DIRECTORY', '<%=type%>', '权限设置Tab');
     <%}%>
...
<body>
<div id="tree-div"
	style="border:1px #ccc solid;width:700px;height:450px;"></div>
</body>
</html>
