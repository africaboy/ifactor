 ...
 * @return {}
 */
function invoice_apply_flow_monitor(value, cellmeta, record,
	...
	if (dataIndex == 'INS_CURRENTSTATUS') {
		if (record.data.INS_STATE == '-1')
			...
			return '<div ext:qtitle="" ext:qtip="'+ifactorTranslator.getLangLabel('ifcommon-language', 'see_process')+'"><a href="javascript:void(0);" ext:qtip="'+ifactorTranslator.getLangLabel('ifcommon-language', 'see_process')+'" onclick="javascript:workflowMonitor(\''+ifactorTranslator.getLangLabel('invoice-language', 'invoice_apply')+'('
		...
		if (record.data['INS_ID'] != '0') {
			return '<div ext:qtitle="" ext:qtip="'+ifactorTranslator.getLangLabel('ifcommon-language', 'see_detail')+'"><a href="javascript:void(0);" ext:qtip="'+ifactorTranslator.getLangLabel('ifcommon-language', 'see_detail')+'" onclick="javascript:workflowTrace(\''+ifactorTranslator.getLangLabel('invoice-language', 'invoice_apply')+'('
					+ record.data['COMPANY_NAME']
 ...
 * @return {}
 */
function invoice_apply_flow_monitor1(value, cellmeta, record,
		rowIndex, columnIndex, store, gridId, options) {
	var dataIndex = getGridCm(gridId).getDataIndex(columnIndex);
...
function invoice_apply_flow_monitor2(value, cellmeta, record,
		rowIndex, columnIndex, store, gridId, options) {
	var dataIndex = getGridCm(gridId).getDataIndex(columnIndex);
