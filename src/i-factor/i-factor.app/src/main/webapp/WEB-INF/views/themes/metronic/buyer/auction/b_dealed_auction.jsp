            ...
            <div class="page-content">
                ...
                <div class="page-bar">
                    <ul class="page-breadcrumb">
                        <li>
                            <a href="#"><fmt:message key='buyer.dealed.auction.page.header.auction' /></a>
                ...
                <h5 class="page-title">
                    <b><fmt:message key='buyer.dealed.auction.page.title.dealed' />&nbsp;&nbsp;&nbsp;<span id="invoiceNum"></span></b>
                </h5>
                <h5 class="page-title">
                    <small><b><fmt:message key='seller.market.list.page.auctionStatus' />:&nbsp;<span style="color: #d64635"><fmt:message key='buyer.dealed.auction.page.Status' />&nbsp;<i class="icon-lock"></i></span></b></small>
                </h5>
                <!-- END PAGE HEADER-->
                <!-- BEGIN PAGE CONTENT-->
                    ...
                    <div class="panel panel-success">
                        ...
                        <div class="panel-body">
                            <div class="row">
                                ...
                                <div class="col-sm-6 col-xs-6 text-right"><a href="#" id="toggle_link" class="if_toggle if_show" onclick="toggle('More&nbsp;&nbsp;', 'Less&nbsp;&nbsp;','col-sm-offset-5 col-sm-1 col-xs-12 text-right','col-sm-offset-9 col-sm-1 col-xs-12 text-right');">More&nbsp;&nbsp;<i class="icon-arrow-down"></i></a></div>
                            </div>
                            <div class="row">
                                ...
                                <div class="col-sm-2 col-xs-12 text-left  if-div-input-sm if-tooltip if_filter_hidden"><a href="#"><fmt:message key='seller.myinvoice.dealed.page.panel.body.acceptedAdvance' /></a></div>
                                ...
                                <div class="col-sm-2 col-xs-12 text-left  if-div-input-sm if-tooltip if_filter_hidden"><a href="#"><fmt:message key='seller.myinvoice.dealed.page.panel.body.acceptedInterest' /></a></div>
                                <div class="col-sm-2 col-xs-12 if_filter_hidden">
                                ...
                                <!-- Begin hidden fields -->
                                <!-- Begin Release Date -->
                                <div class="col-sm-2 col-xs-12 text-left  if-div-input-sm if_filter_hidden"><a href="#"><fmt:message key='invoice.list.thead.releaseDate' /></a></div>
                                <div class="col-sm-2 col-xs-12 if_filter_hidden">
                                ...
                                <!-- Begin my bid -->
                                <div class="col-sm-2 col-xs-12 text-left  if-div-input-sm if_filter_hidden"><a href="#"><fmt:message key='buyer.mybid.dealed.page.header.myBids' /> &nbsp;<span><input type="checkbox" id="myBids" name="myBids" value="My Bids"></span></a></div>
                                <!-- End my bid -->
                                <!-- End hidden fields -->
                                ...
                                <table class="table table-striped table-bordered table-hover"  id="content_table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="width: 100px" class="if-tooltip">
                                                <a ata-toggle="tooltip white" data-original-title="Ready to sell interest" style="color:white"><fmt:message key='seller.market.list.page.panel.body.rtsi' />&nbsp;<i class="icon-question"></i></a>
                                            </th>
                                            <th style="width:20px">
                                    ...
                                    <%--  <c:if test="${not empty page.content}">
                        			  <c:forEach var="item" items="${page.content}" varStatus="status">
                        			    <c:if test="${userId==item.buyerId}">
				                          <tr class="success ifactive odd">
				                        </c:if>
				                        <c:if test="${userId!=item.buyerId}">
				                          <tr>
				                        </c:if>
				                          ...
				                          <td>
				                            <a href="javascript:;" target="main-iframe">
			                          	      <button type="button" class="btn-small btn-info btn-circle" onclick="window.open('${base}/buyer/auction/dealed/biddhistory/${item.id}');"><i class="fa fa-list"></i></button>
				                            </a>
				                          </td>
				                          <td><fmt:formatDate value='${item.finInvIssDate}' pattern='MM/dd/yyyy'/></td>
				                          <c:if test="${userId==item.buyerId}">
				                            <td class="if-tooltip">
                                              <a data-toggle="tooltip" data-original-title="My bid">${item.finStickerId}</a><img style="padding-left: 2px" src="${base}/static/themes/metronic/img/mybid.png"><!--<span class="flaticon-law"></span>-->
                                            </td>
				                          </c:if>
				                          <c:if test="${userId!=item.buyerId}">
				                          ...
				                          <td>
				                            <c:if test="${item.noOfOffers==null}">0</c:if>
				                            <c:if test="${item.noOfOffers!=null}">${item.noOfOffers}</c:if>
				                          </td>
				                        </tr>
				                      </c:forEach>
				                     </c:if> --%>
    ...
    <script>
        jQuery(document).ready(function () {
            ...
            $('#content_table').DataTable({
               ...
               sAjaxSource: app.base + '/buyer/auction/dealed/list',
                	...
                	{ "mData": "noOfOffers", 'sClass':'left'}
                	  ...
                	  "fnRowCallback": function(nRow, aData, iDisplayIndex) {// 当创建了行，但还未绘制到屏幕上的时候调用，通常用于改变行的class风格 
                        ...
                        /*   if (aData.status != 1 && aData.status != 4 && aData.active == 0) {  */
                        	  ...
                        	  var td1 = "<a href='${base}/buyer/auction/dealed/biddhistory/"+aData.id+"' target='main-iframe'><button type='button' class='btn-small btn-info btn-circle'> <i class='fa fa-list'></i>"+"</button></a>";
                              var td2 = fDate.Format('MM/dd/yyyy');
                              if(aData.buyerId==${loginUser.id}){
                                var td3 = "<a data-toggle='tooltip' data-original-title='My bid'>"+aData.finStickerId+"</a><img style='padding-left: 2px' src='${base}/static/themes/metronic/img/mybid.png'>";
                              }else{
                                var td3 =  aData.finStickerId; 
                              }
                              var td4 = decimalInputFormatFunc(aData.finInvAmount)+"&nbsp;VND";
                              var td5 = "<a href='${base}/buyer/profile/seller/"+aData.sellerId+"'> "+aData.sellerId+" </a>";
                              var td6 = "<a href='${base}/buyer/profile/debtorById/"+aData.id+"'> "+aData.debtorName+" </a>";
                              var td7 = rDate.Format('MM/dd/yyyy');
                              ...
                              if(aData.noOfOffers==null){
                                  var td12 = 0+"&nbsp;<fmt:message key='b.make.new.offer.page.bid' />";
                               }else if(aData.noOfOffers==1){
                                  var td12 = 1+"&nbsp;<fmt:message key='b.make.new.offer.page.bid' />";
                               }else{
                             	 var td12 = aData.noOfOffers+"&nbsp;<fmt:message key='b.make.new.offer.page.bids' />";
                               }
                              $('td:eq(0)', nRow).html(td0);
                              $('td:eq(1)', nRow).html(td1);
                              $('td:eq(2)', nRow).html(td2);
                              $('td:eq(3)', nRow).html(td3);
                              $('td:eq(3)', nRow).addClass("if-tooltip");
