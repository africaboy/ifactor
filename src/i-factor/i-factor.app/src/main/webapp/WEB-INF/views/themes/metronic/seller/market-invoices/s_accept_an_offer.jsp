...
<div class="page-bar">
  <ul class="page-breadcrumb">
    ...
    <li>
      <a href="javascript:;"><fmt:message key="seller.acceptAnOffer.page.head.acceptAnOffer"/></a>
    </li>
  </ul>
</div>
<h5 class="page-title">
  <!--<small><b>Auctions status:  <span style="color: green">OPEN <i class="icon-lock-open"></i></span></b></small>-->
  <b><fmt:message key="seller.acceptAnOffer.page.title"/>&nbsp;${invoice.id}</b>
</h5>
<!-- END PAGE HEADER-->
...
<div class="clearfix">
  ...
  <span class="if-panel-next_to"> <a href="${base}/seller/profile/debtorById/${invoice.id}">&gt;&gt;&gt; <fmt:message key="seller.acceptAnOffer.page.panel.body.viewMore"/>&nbsp;&nbsp;<i class="icon-arrow-right"></i></a></span>
  <div class="panel panel-success">
    <div class="panel-body">
      <div class="row">
        <!-- Row 1 -->
  ...
  <div class="panel panel-success">
    <div class="panel-body">
      <div class="row">
        ...
        <div class="col-sm-2 col-xs-12">
          <input type="text" class="form-control if-input-sm if-div-input-sm" readonly  id="txtAdvance1" value="${invoice.readyToSellAdv}&nbsp;%">
          <input type="hidden" id="txtAdvance" value="${invoice.readyToSellAdv}">
        </div>
        ...
        <div class="col-sm-2 col-xs-12 ">
          <input type="text" class="form-control if-input-sm if-div-input-sm" readonly  id="txtInterest1" value="${invoice.readyToSellInt}&nbsp;%"></span>
          <input type="hidden" id="txtInterest" value="${invoice.readyToSellInt}">
        </div>
...
<div class="clearfix">
  <div class="if-panel-head if-panel-green">
    <fmt:message key="seller.acceptAnOffer.page.panel.title.acceptAnOffer"/>
  </div>
  <div class="panel panel-success">
   <form  class="form-horizontal"  id="form-bid-info"  method="post">
	...
	<input type="hidden"  id="bestAdvance"  value="${invoice.bestBuyerAdv}">
	<input type="hidden"  id="bestInterest" value="${invoice.bestBuyerInt}">
	<input type="hidden"  id="txtExpectSettle" value="${invoice.finExpPmtDate}">
    <div class="panel-body">
      <div class="row">
        <div class="col-sm-2 col-xs-12 text-left if-div-input-sm"><a href="javascript:;"><fmt:message key="seller.acceptAnOffer.page.panel.body.advance"/></a></div>
        <div class="col-sm-2 col-xs-12">
         <c:if test="${invoice.bestBuyerAdv==null}">
         <input type="text" class="form-control if-input-sm if-div-input-sm" readonly  id="bestAdvance1" value="">
         </c:if>
         <c:if test="${invoice.bestBuyerAdv!=null}">
         <input type="text" class="form-control if-input-sm if-div-input-sm" readonly  id="bestAdvance1" value="${invoice.bestBuyerAdv}&nbsp;%">
         </c:if>
         <input type="hidden" id="discount" name="discount" value="${invoice.bestBuyerAdv}">
        </div>
        <div class="col-sm-2 col-xs-12 text-left if-div-input-sm"><a href="javascript:;"><fmt:message key="seller.acceptAnOffer.page.panel.body.interest"/></a></div>
        <div class="col-sm-2 col-xs-12 ">
        	 <c:if test="${invoice.bestBuyerAdv==null}">
	         <input type="text" class="form-control if-input-sm if-div-input-sm" readonly  id="bestInterest1" value="">
	         </c:if>
	         <c:if test="${invoice.bestBuyerAdv!=null}">
	         <input type="text" class="form-control if-input-sm if-div-input-sm" readonly  id="bestInterest1" value="${invoice.bestBuyerInt}&nbsp;%">
	         </c:if>
             <input type="hidden" id="interest"  name="interest" value="${invoice.bestBuyerInt}">
        </div>
        <div class="col-sm-4 col-xs-12 if-div-input-sm">
        ...
        <div class="col-sm-12 text-center" style="margin-top: 10px">
          <a href="javascript:;" class="btn if-detail-btn-group if-btn-medium dialog_accept_offer" <c:if test="${invoice.bestBuyerAdv==null}">disabled=disabled</c:if>>
            <fmt:message key="seller.acceptAnOffer.page.button.acceptOffer"/>
          </a>
        </div>
      </div>
    </div>
...
<script src="${base}/static/themes/metronic/js/seller/my-invoices/aler-dialog-api.js"></script>
...
<script src="${base}/static/themes/metronic/js/ifactoracceptoffer.js"></script>
...
<script>
  ...
  var app = {
			...
			selfUrl : '${self.url}',
			titleOfferRts : "<fmt:message key='s.reject.an.offer.page.pleaseConfirm'/> <b><fmt:message key='s.reject.an.offer.page.accept'/></b> <fmt:message key='s.reject.an.offer.page.theFollowing'/>&nbsp;${invoice.id}:",
			noOffer: "<fmt:message key='s.accept.offer.page.noOffer'/>",
		    submitMsg : "<fmt:message key='s.accept.offer.page.submit.msg'/>",
            submitTitle : "<fmt:message key='s.accept.offer.page.submit.title'/>",
			strOfferRts : "<fmt:message key='b.make.str.make.an.offer1'/>&nbsp;<fmt:formatNumber value="${invoice.finInvAmount}" type="number" />&nbsp;<fmt:message key='b.make.str.make.an.offer2'/>&nbsp;${invoice.bestBuyerAdv}&nbsp;%<fmt:message key='b.make.str.make.an.offer3'/>&nbsp;${invoice.bestBuyerInt}&nbsp;%<fmt:message key='b.make.str.make.an.offer4'/>",
