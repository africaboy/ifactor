$(function() {	
	$('#saveApplication').click(function() {
		...
		var url = app.base + '/buyer/account/apply?isCommit=false';
	...
	$('#submitApplication').click(function(e) {
		...
		var formIds = [ 'form-personal-info', 'form-company-info','form-further-details', 'form-file-hidden','form-terms-conditions' ];
		var formData = MergeFormData(formIds);
		var url = app.base + '/buyer/account/apply?isCommit=true';
		AjaxPostRequest(url, formData, successSaveOrSubmit);
	});
	...
	$('#form-personal-info').validate({
		errorPlacement: function (error, element) {
            ...
            if(dateInputId=='dob'||dateInputId=='workCountryCode') {
	...
	$('#isComVietnam').on('change',function() {
		if($('#isComVietnam').val()=='No'){
			$('#comRegisteredCountry').attr('disabled', false);
		}else{
			$('#comRegisteredCountry').val('');
			$('#comRegisteredCountry').attr('disabled', true);
		}
	});
	
	$('#haveEverTradingOther').on('change',function() {
		if($('#haveEverTradingOther').val()=='Yes'){
			$('#whatIsAsset').attr('disabled', false);
		}else{
			$('#whatIsAsset').val('');
			$('#whatIsAsset').attr('disabled', true);
		}
	});
	...
	$('#isComAddress2').click(function(){
		if($("#isComAddress2").attr("checked")=="checked"){
			$('#different').show();
    ...
    $('#dob,#authzDob').datepicker({
        ...
        defaultDate: -365*18,
        showOn: "button",
        buttonImage: app.base+"/static/libs/metronic/3.3.0/assets/admin/pages/img/calendar.png",
        buttonImageOnly: true,
        buttonText: "Select date"
    });
	
	$('#comEstablishmentDate,#dob,#authzDob').on('change',function(e){
...
function initUploadSettings(fileinput_id) {
	...
	$('#' + fileinput_id).fileupload({
		...
		dataType : 'text',
			...
			$('#' + filelink_id).html('<a href="' + app.base + '/fileDownload/'+ biztype + '/' + data.result+ '" title="'+originalName+'">' + showname + '</a>').css({});
...
function successSaveOrSubmit(result) {
	...
	if (result.success) {
		...
		var msgTpl = app.successRedirectMsg;
		var redUrl = app.base+'/';
