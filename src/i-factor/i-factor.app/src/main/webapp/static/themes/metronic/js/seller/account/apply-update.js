$(function (){
	$('#submitApplication').click(function(e){
		...
		var url = app.base + '/seller/account/updateapp?isCommit=true';
	...
	$('#saveApplication').click(function(e){
		...
		*/
		var formIds = ['form-personal-info','form-company-info','form-further-details','form-documents-hidden','form-terms-conditions']
		var formData = MergeFormData(formIds);
		var url = app.base + '/seller/account/updateapp?isCommit=false';
	...
	$('#isComVietnam').on('change',function() {
		if($('#isComVietnam').val()=='Yes'){
			$('label[for="comRegisteredCountry"]').html('');
			$('#comRegisteredCountry').rules('remove');
			$('#comRegisteredCountry').prop("disabled", true);
			
		}else{
			$('#comRegisteredCountry').prop("disabled", false);
    ...
    $('#comEstablishmentDate,#dob').on('change',function(e){
